...
```
30,000석 조회 → JSON 직렬화 30,000건 → 응답 전송
→ 평균 48,990ms
```
...
| 방식1 적용 인덱스 적용 전 (10,000석) | 23,816ms | 약   51.4% 개선 |
| 방식1 후 인덱스 적용 후 (10,000석)  | 16,450ms | 약 30.9% 개선   |
...
|:---|:---|:---|
| **인덱스 적용 전** | 23,816ms | - |
| **인덱스 적용 후** | 16,450ms | **약 30.9% 개선** |

### 전체 개선 요약

```
48,990ms (방식1 전)
    ↓  구역별 분리 조회 적용 (51.4% 개선)
23,816ms (방식1 후, 인덱스 전)
    ↓  인덱스 추가 (30.9% 추가 개선)
